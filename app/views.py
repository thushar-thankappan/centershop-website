...
from .models import Customer, Product, Cart, OrderPlaced # models
...
class ProductView(View):
	def get(self, request):
		totalitem = 0
		suggested = Product.objects.filter(category='S')
		recommended = Product.objects.filter(category='R')
		topwears = Product.objects.filter(category='TW')
		bottomwears = Product.objects.filter(category='BW')
		mobiles = Product.objects.filter(category='M')
		if request.user.is_authenticated:
			totalitem = len(Cart.objects.filter(user=request.user))
		return render(request, 'app/home.html', {'suggested': suggested, 'recommended': recommended,'topwears':topwears, 'bottomwears':bottomwears, 'mobiles':mobiles, 'totalitem':totalitem})
...
def topwear(request, data=None):
	...
	if data==None :
			topwears = Product.objects.filter(category='TW')
	elif data == 'Shirt' or data == 'Jacket':
...
def suggest(request, data=None):
	totalitem = 0
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user))
	if data==None :
			suggests = Product.objects.filter(category='S')
	elif data == 'swatch' or data == 'mcflower':
			suggests = Product.objects.filter(category='S').filter(brand=data)
	elif data == 'below':
			suggests = Product.objects.filter(category='S').filter(discounted_price__lt=500)
	elif data == 'above':
			suggests = Product.objects.filter(category='S').filter(discounted_price__gt=500)
	return render(request, 'app/suggest.html', {'suggests':suggests, 'totalitem':totalitem})

def recommend(request, data=None):
	totalitem = 0
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user))
	if data==None :
			recommends = Product.objects.filter(category='R')
	elif data == 'Clock' or data == 'Bag':
			recommends = Product.objects.filter(category='R').filter(brand=data)
	elif data == 'below':
			recommends = Product.objects.filter(category='R').filter(discounted_price__lt=400)
	elif data == 'above':
			recommends = Product.objects.filter(category='R').filter(discounted_price__gt=400)
	return render(request, 'app/recommend.html', {'recommends':recommends, 'totalitem':totalitem})
